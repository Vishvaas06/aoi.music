...
class AudioPlayer {
    ...
    async play(emit = true) {
        ...
        let stream;
        //@ts-ignore
        stream = await (0, request_1.requestStream)(current, current?.formattedPlatforms ?? "Youtube", this.options.manager);
        let s;
        ...
        if (this.options.manager.plugins.has(enums_1.PluginName.Filter) && __classPrivateFieldGet(this, _AudioPlayer_modes, "f").filters.length) {
        ...
        emit && this.options.manager.emit(enums_1.PlayerEvents.TrackStart, current, this);
        this.player.play(resource);
        if (__classPrivateFieldGet(this, _AudioPlayer_modes, "f").ytMix) {
            if (this.queue[__classPrivateFieldGet(this, _AudioPlayer_modes, "f").currentTrack].id === __classPrivateFieldGet(this, _AudioPlayer_modes, "f").ytMix.lastUrl) {
    ...
    _configPlayer() {
        this.player.on("stateChange", async (os, ns) => {
            if (os.status !== voice_1.AudioPlayerStatus.Idle && ns.status === voice_1.AudioPlayerStatus.Idle) {
                if (__classPrivateFieldGet(this, _AudioPlayer_modes, "f").paused) {
                }
                else if (__classPrivateFieldGet(this, _AudioPlayer_modes, "f").loop === enums_1.LoopMode.Track && this.queue[__classPrivateFieldGet(this, _AudioPlayer_modes, "f").currentTrack]) {
                ...
                else if (__classPrivateFieldGet(this, _AudioPlayer_modes, "f").loop === enums_1.LoopMode.Queue && this.queue.length) {
                    if (__classPrivateFieldGet(this, _AudioPlayer_modes, "f").seeked) {
                        __classPrivateFieldGet(this, _AudioPlayer_modes, "f").seeked = false;
                        return;
                    }
                ...
                else if (__classPrivateFieldGet(this, _AudioPlayer_modes, "f").autoPlay != "none" && this.queue.length === 1) {
                    if (__classPrivateFieldGet(this, _AudioPlayer_modes, "f").seeked) {
                        __classPrivateFieldGet(this, _AudioPlayer_modes, "f").seeked = false;
                        return;
                    }
                ...
                else if (this.queue.length > 1 && __classPrivateFieldGet(this, _AudioPlayer_modes, "f").currentTrack < this.queue.length - 1) {
                    if (__classPrivateFieldGet(this, _AudioPlayer_modes, "f").seeked) {
                        __classPrivateFieldGet(this, _AudioPlayer_modes, "f").seeked = false;
                        return;
                    }
            ...
            if (os.status === voice_1.AudioPlayerStatus.Playing && ns.status !== voice_1.AudioPlayerStatus.Playing && ns.status !== voice_1.AudioPlayerStatus.Idle && ns.status !== voice_1.AudioPlayerStatus.Paused) {
                if (__classPrivateFieldGet(this, _AudioPlayer_modes, "f").seeked) {
                    __classPrivateFieldGet(this, _AudioPlayer_modes, "f").seeked = false;
                    return;
                }
    ...
    async add(track, type, member) {
        for (let i = 0; i < track.length; i++) {
            if (type === enums_1.PlatformType.Youtube) {
                ...
                if (track[i].includes("&list=") && track[i].includes("&index=") && track.includes("watch?v=") && !__classPrivateFieldGet(this, _AudioPlayer_modes, "f").ytMix.enabled) {
                ...
                this.options.manager.emit(enums_1.PlayerEvents.TrackAdd, info, this);
                ...
                const info = await (0, request_1.requestInfo)(track[i], constants_1.formattedPlatforms[enums_1.PlatformType.SoundCloud], this.options.manager);
                if (!info)
                    continue;
                ...
                const info = await (0, request_1.requestInfo)(track[i], constants_1.formattedPlatforms[enums_1.PlatformType.Url], this.options.manager);
                this.queue.push({
                    ...info,
                    requester: member,
                    position: this.queue.length
    ...
    async autoPlayNext() {
        if (this.autoPlay === enums_1.AutoPlay.Youtube || this.autoPlay === enums_1.AutoPlay.Spotify) {
            ...
            if (this.currentTrack.formattedPlatforms.toLowerCase() === enums_1.AutoPlay.Youtube /*|| this.currentTrack.formattedPlatforms.toLowerCase() === AutoPlay.Spotify*/) {
                const parsed = await (await (0, undici_1.fetch)(`https://youtube.com/watch?v=${this.currentTrack.id}`)).text();
                const data = (0, helpers_1.ytRelatedHTMLParser)(parsed);
                const ids = (0, helpers_1.YoutubeRelated)(data);
                for (const id of ids) {
                    const info = await (0, request_1.requestInfo)(id, this.currentTrack.formattedPlatforms.toLowerCase() === "youtube" ? "Youtube" : "Spotify", this.options.manager);
                    if (!info)
                        continue;
                    this.queue.push({
                        ...info,
            ...
            else if (this.currentTrack.formattedPlatforms.toLowerCase() === enums_1.AutoPlay.SoundCloud) {
                const sc = this.options.manager.platforms.soundcloud;
                //@ts-ignore
                const { collection: data } = await sc.related(
                //@ts-ignore
        ...
        this.options.connection.on("stateChange", async (_, newState) => {
            if (newState.status === voice_1.VoiceConnectionStatus.Disconnected) {
                if (newState.reason === voice_1.VoiceConnectionDisconnectReason.WebSocketClose && newState.closeCode === 4014) {
                    try {
                        await (0, voice_1.entersState)(this.options.connection, voice_1.VoiceConnectionStatus.Connecting, 5_000);
                     ...
                     */
                    await (0, promises_1.setTimeout)((this.options.connection.rejoinAttempts + 1) * 5_000);
            ...
            else if (newState.status === voice_1.VoiceConnectionStatus.Connecting || newState.status === voice_1.VoiceConnectionStatus.Signalling) {
                /**
                 ...
                 */
                try {
                    await (0, voice_1.entersState)(this.options.connection, voice_1.VoiceConnectionStatus.Ready, 20_000);
                }
                catch {
                    if (this.options.connection.state.status !== voice_1.VoiceConnectionStatus.Destroyed) {
                        this._destroy();
                        this.options.connection.destroy();
                    }
                }
    ...
    getQueue(page = 1, limit = 10, format = "{position}) {title} | {requester.user.username}") {
        ...
        let tracks = this.loop === enums_1.LoopMode.Queue ? [...nextTracks, ...prevTracks] : nextTracks;
